...
namespace Building
{
    public partial class mainForm : Form
    {
        ...
        private void initializeTableList()
        {
            ...
            string cmdStr = "SHOW TABLES FROM vetklin1";
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    if (user == 2)
                    {
                        ...
                        if (reader.GetString(0) == "специализация_врачей" || reader.GetString(0) == "должности" || reader.GetString(0) == "команды_врачей" || reader.GetString(0) == "сотрудники_клиники")
                        {
                            continue;
                        }
                    }
        ...
        private void mainForm_Text(object sender, EventArgs e)
        {
            ...
            this.Text = "БД вет.клиники." + temp.ToUpper();
        ...
        {
            string queryStr = string.Format("SELECT SUM(Стоимость) FROM текущие_услуги WHERE Дата_сеанса BETWEEN '{0}' AND '{1}'",
